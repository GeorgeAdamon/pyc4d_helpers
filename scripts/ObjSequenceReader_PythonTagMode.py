...
def SetupUserData():
    ...
    if not ud.UserDataExists(op.GetObject(),"File Handling"):
        ud.CreateUserData(op.GetObject(),"File Handling", c4d.DTYPE_SEPARATOR, True)
    #2
    if not ud.UserDataExists(op.GetObject(),"First Object FileName"):
        ud.CreateUserData(op.GetObject(),"First Object FileName", c4d.DTYPE_FILENAME, True)
    #3
    if not ud.UserDataExists(op.GetObject(),"First Frame"):
        ud.CreateIntegerData(op.GetObject(),"First Frame", "Integer", 0,100,1)
    #4
    if not ud.UserDataExists(op.GetObject(),"Last Frame"):
        ud.CreateIntegerData(op.GetObject(),"Last Frame", "Integer", 0,100,1)
    #5
    if not ud.UserDataExists(op.GetObject(),"Frame Step"):
        ud.CreateIntegerData(op.GetObject(),"Frame Step", "Integer Slider", 1,100,1)
        ud.SetUserData(op.GetObject(), "Frame Step", 1)
    #6
    if not ud.UserDataExists(op.GetObject(),"File Handling Message"):
        ud.CreateUserData(op.GetObject(),"File Handling Message", c4d.DTYPE_STATICTEXT, True)
    #7
    if not ud.UserDataExists(op.GetObject(),"Mesh Handling"):
        ud.CreateUserData(op.GetObject(),"Mesh Handling", c4d.DTYPE_SEPARATOR, True)
    #8
    ...
    if not ud.UserDataExists(op.GetObject(),"Phong Smoothing"):
        ud.CreateUserData(op.GetObject(),"Phong Smoothing", c4d.DTYPE_BOOL, True)
    #10
    if not ud.UserDataExists(op.GetObject(),"Import Vertex Colors"):
        ud.CreateUserData(op.GetObject(),"Import Vertex Colors", c4d.DTYPE_BOOL, True)
    #11
    if not ud.UserDataExists(op.GetObject(),"Create Test Material"):
        ud.CreateUserData(op.GetObject(),"Create Test Material", c4d.DTYPE_BOOL, True)
    #12
    if not ud.UserDataExists(op.GetObject(),"Mesh Handling Message"):
        ud.CreateUserData(op.GetObject(),"Mesh Handling Message", c4d.DTYPE_STATICTEXT, True)
...
def ResolveFilename(filename):
    ...
    if last <= first:
        frame = max(frame,first) #op.GetObject()en ended sequence, no max limit
    else:
...
def ImportToCinema():
        ...
        PATH =  ResolveFilename(ud.GetUserDataValue(op.GetObject(), "First Object FileName"))
        
        N = os.path.basename(PATH)
        
        if PATH != None:
            Vertices, Faces, Colors = ParseObj(PATH, ud.GetUserDataValue(op.GetObject(), "Swap Y/Z")) #LOAD OBJ FILE
            
            children = op.GetObject().GetChildren()

            if len(children)==0:
                Polygon = c4d.BaseObject(c4d.Opolygon) #Create an empty polygon object
...
def main():
    op.GetObject().SetName("--- OBJ Sequence Reader by George Adamon ---")
    ImportToCinema()
